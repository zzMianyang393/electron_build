...
function ensureNodeModules(currentAppPath, backupPath) {
  const currentModules = path.join(currentAppPath, "node_modules");
  const backupModules = path.join(backupPath, "node_modules");

  if (!fs.existsSync(currentModules) && fs.existsSync(backupModules)) {
    copyDirSync(backupModules, currentModules);
  }
}
...
function replaceApp(currentAppPath, updatePayloadPath) {
  ...
  ensureNodeModules(currentAppPath, backupPath);
  removeDirSync(backupPath);
}
